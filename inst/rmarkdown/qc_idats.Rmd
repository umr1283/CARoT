...
1. Filtering Probes
    * `filter_snps` ([Zhou et al., 2016](https://www.doi.org/10.1093/nar/gkw967))  
        => remove probes in which the probed CpG falls near a SNP
    * `filter_non_cpg`  
...
1. Filtering Probes
    ...
    * __Call Rate__  
        => remove probes with less than the specified call rate (`r params[["callrate_probes"]]`) for detection p-values below $\alpha=`r params[["detection_pvalues"]]`$
1. Filtering Samples
    * __Call Rate__  
        => remove samples with less than the specified call rate (`r params[["callrate_samples"]]`) for detection p-values below $\alpha=`r params[["detection_pvalues"]]`$
    * __Gender~[Phenotype]~ $\neq$ Gender~[Probes]~__  
