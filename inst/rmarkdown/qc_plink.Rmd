...
* `r scales::comma(nrow(recruit_centre))` samples (table \@ref(tab:cohort-overview-table)).  
...
Figure \@ref(fig:sample-call-rate-05) shows the call rate per sample.  
...
gender_check_03_caption <- paste(
  ...
  "Samples for which gender information is discordant or missing were denoted by red cross."
)
```
...
Figure \@ref(fig:gender-check-03) illustrates the homozygosity estimate against 
...
```{r gender-check-03, eval = gender_file_check, fig.cap = gender_check_03_caption}
ggplot2::ggplot(
...
```{r heterozygosity-05, fig.height = 4.7 * 1.5, fig.cap = heterozygosity_caption}
ggpubr::ggarrange(
  p_heteR + 
...
Table \@ref(tab:relatedness-02) shows the distribution of pi-hat (proportion of IBD) for related samples. 
...
```{r imendel-error-03, eval = params[["includes_relatives"]] & imendel_file_check, fig.cap = imendel_error_03_caption}
ggplot2::ggplot(
...
```{r population-03, fig.height = 4.7 * 1.5, fig.cap = population_03_caption}
...
the first two principal components), this is illustrated in Figure \@ref(fig:population-06).

```{r population-06, fig.cap = population_06_caption}
...
```{r population-07, fig.cap = population_07_caption}
...
Figure \@ref(fig:variant-call-rate-02) shows the proportion of missing genotypes per variant. 
...
```{r variant-call-rate-02, fig.cap = variant_call_rate_02_caption}
ggplot2::ggplot(
...
```{r lmendel-error-02, eval = params[["includes_relatives"]] & lmendel_file_check, fig.cap = lmendel_error_02_caption}
ggplot2::ggplot(
...
The observed MAF distribution is shown in Figure \@ref(fig:maf-02).  
...
```{r maf-02, fig.cap = maf_02_caption}
...
Figure \@ref(fig:hwe-02) shows variants with 
...
```{r hwe-02, fig.cap = hwe_02_caption}
...
* Samples previously characterised in Section \@ref(sample-qc) (table \@ref(tab:qc-summary-table)):
...
output_xlsx[["exclude_flag"]] <- samples_toexclude_export %>%
  ...
  dplyr::rename(
    "Ethnicity" = "PCA Outliers (non-targeted population)",
