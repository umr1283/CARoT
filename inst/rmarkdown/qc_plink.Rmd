...
```{r sample_call_rate_01, include = FALSE}
system(command = paste(
  params[["bin_path"]][["plink"]], 
...
```{r gender_check_02, eval = gender_file_check, include = FALSE}
...
```{r heterozygosity_01, include = FALSE}
system(command = paste(
  params[["bin_path"]][["plink"]], 
  "--bfile", paste0(temp_directory, "/raw_data"), 
  "--het",
...
```{r heterozygosity_02, include = FALSE}
...
```{r Relatedness_01, eval = !params[["includes_relatives"]], include = FALSE}
## Pruning Data
system(command = paste(
  params[["bin_path"]][["plink"]], 
  "--bfile", paste0(temp_directory, "/raw_data"), 
...
Figure \@ref(fig:imendel_error_04) shows the Mendel error per sample.  
`r scales::comma(nrow(mendel_errors))` samples (from `r dplyr::n_distinct(mendel_errors[["FID"]])` families)
had a Mendel error rate higher than `r scales::percent(x = params[["mendelian_samples"]], accuracy = 0.01)`
and were flagged (table \@ref(tab:imendel_error_05)).

```{r imendel_error_03, eval = params[["includes_relatives"]] & imendel_file_check}
...
imendel_error_04_caption <- "Sample Mendel error rate."
```

```{r imendel_error_04, eval = params[["includes_relatives"]] & imendel_file_check, fig.cap = imendel_error_04_caption}
ggplot2::ggplot(
...
```{r imendel_error_05, eval = params[["includes_relatives"]] & imendel_file_check}
...
```{r variant_call_rate_01, include = FALSE}
system(command = paste(
  params[["bin_path"]][["plink"]], 
...
```{r duplicated_snps, include = FALSE}
...
```{r uniq_snps_list, include = FALSE}
if (lmendel_file_check) {
  cat(
    setdiff(
      uniq_snps[["SNP"]], 
...
maf_02_caption <- "Distribution of variants number per class of Minor Allele Frequency (MAF)"
```
...
```{r output_exclusion, include = FALSE}
output_xlsx <- list()
...
```{r softwares_version, include = FALSE}
plink_ver <- strsplit(
