...
pairwise IBD (identity by descent) are computed, using the `--genome` command in PLINK.
...
The data from the studied population (*i.e.*, **`r params[["cohort_name"]]`**)  pruned using the following cut-offs:
...
The resulting "clean" dataset (*i.e.*, `r scales::comma(n_samples-n_samples_1kg)` samples and `r scales::comma(n_good_snp)` variants) were merged with the data from the publicly available *1,000 Genomes project* database, 
...
data_pca_plot <- data_pca %>%
  ...
  dplyr::mutate(is_kept = ifelse(is.na(QC), "KEPT", "EXCLUDED"))
...
```{r population-03a, fig.height = 4.7 * 1.5, fig.cap = paste("First factorial plane computed with a combined dataset", "involving samples from the 1,000 Genomes Project.", 'Samples were coloured by "super population" groups.')}
p_superpop  +
  ggplot2::facet_grid(
    rows = ggplot2::vars(Cohort), 
    cols = ggplot2::vars(is_kept)
  )
```

```{r population-03b, fig.height = 4.7 * 1.5, fig.cap = paste("First factorial plane computed with a combined dataset", "involving samples from the 1,000 Genomes Project.", 'Samples were coloured by "population" groups.')}
p_pop +
  ggplot2::facet_grid(
    rows = ggplot2::vars(Cohort), 
    cols = ggplot2::vars(is_kept)
