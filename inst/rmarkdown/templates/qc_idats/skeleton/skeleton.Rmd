...
sample_sheet <- readr::read_csv(params[["csv_file"]], skip = 8) %>% 
  dplyr::mutate(
    gender_clean = if (do_check_gender) {
      c("1" = 1, "2" = 2, "M" = 1, "F" = 2, "0" = 2)[!!!params[["gender_colname"]]]
    } else {
      NULL
    }
  ) %>% 
  dplyr::select(Sample_ID = Sample_Name, dplyr::everything())
...
if (tolower(params[["cell_tissue"]]) %in% "blood" & !requireNamespace(paste0("FlowSorted.Blood.", params[["array"]]))) {
  do_cell_composition <- FALSE
...
if (tolower(params[["cell_tissue"]]) %in% "cordblood" & !requireNamespace("FlowSorted.CordBloodCombined.450k")) {
...
if (!tolower(params[["cell_tissue"]]) %in% c("blood", "cordblood") & !requireNamespace("RefFreeEWAS")) {
  do_cell_composition <- FALSE
...
```{r export}
readr::write_rds(x = data_mset, path = file.path(output_directory, paste0(params[["array"]], "_mset.rds")))
```
